...
<div class="row">
    ...
    <div class="header"  style="text-align: center">
        ...
        <i style="font-size: 14px;  text-transform: capitalize; "> 
            @if ($kepala->types=="AWAL")
                ( Renstra {{$kepala->yearfrom}}-{{$kepala->yearto}} BBPOM di Banjarmasin )
            @else
                ( {{$kepala->types}} Renstra {{$kepala->yearfrom}}-{{$kepala->yearto}} BBPOM di Banjarmasin Tahun
                    @php
                        $tahunini = date('Y', strtotime($kepala->dates));
                    @endphp
                    {{$tahunini}}
                )
            @endif
    ...
    <table id="isi" style="width: 100%">
        <thead>
            <tr>
                ...
                <th colspan="5">Target BBPOM di Banjarmasin</th>
            </tr>
            <tr>
                @foreach ($thn as $item)
                <th>{{$item->tahun}}</th>
        ...
        <tbody class="tbl">
            ...
            @foreach ($indi as $key=>$row)
                <tr style="text-align: center">
                    ...
                    @foreach ($thn as $item)
                        @php
                            $nilai =$injectQuery->getRenstrakal($kepala->id,$item->tahun, $row->id);
                        @endphp
